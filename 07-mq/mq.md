...
>   目前业界使用较多的一共有四种MQ：ActiveMQ、Kafka、RabbitMQ、RocketMQ。
>
>   *   ActiveMQ
>
>       由于ActiveMQ目前并不是很活跃了，社区不如另外三种，而且用户量也不如另外三种，这里就不做深入对比了。
>
>   *   Kafka
>
>       *   优点：
>           1.   高吞吐量，一台普通的机器就能抗住十几万的QPS
>           2.   高性能，发送消息基本都是毫秒级别的
>           3.   支持集群，保证高可用
>       *   缺点：
>           1.   容易造成数据丢失，不安全，因为它在接收消息的时候并不是直接写到物理磁盘的，而是先写入磁盘缓冲区里面的
>           2.   功能单一，主要就是收发消息，没其他的高级功能，适用场景受限
>       *   业界一般将kafka用来处理用户的行为日志，用在大数据团队居多，可以接受数据的丢失，而且要求吞吐量高。
>
>   *   RabbitMQ
>
>       *   优点：
>           1.   能保证数据不丢失，相当可靠
>           2.   有其他额外的功能，例如消息重试、死信队列
>           3.   支持集群，保证高可用
>           4.   学习成本低，有部分公司以前用的ActiveMQ，后来转为RabbitMQ，遇到了问题在社区中也有很成熟的解决方案
>       *   缺点：
>           1.   吞吐量较低，一般只有几万的QPS，对于大型秒杀等场景就不能胜任
>           2.   集群线性扩展比较麻烦
>       *   如果公司的业务比较平稳，未来几年内不会出现飞速发展，那么可以选用RabbitMQ，这款MQ还是深受中小型企业欢迎的。
>
>   *   RocketMQ
>
>       *   优点：
>           1.   阿里巴巴开源的消息中间件，几乎同时解决了Kafka和RabbitMQ两者的缺点
>           2.   吞吐量很高，普通机器也能有十万的QPS
>           3.   高性能、支持高可用，线性扩展方便
>           4.   支持各种高级功能，如延迟消息、消息回溯等
>       *   缺点：
>           1.   文档没RabbitMQ和Kafka全，写的稍微简单了些
>           2.   没有明显缺点
>       *   如果公司发展迅猛，或者有高并发场景，则可以直接一步到位选择RocketMQ，这样会省很多事情。
