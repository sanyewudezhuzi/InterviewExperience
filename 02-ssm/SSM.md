...
>   *   过滤器只在servlet前后起作用，而拦截器能够深入到方法前后、异常抛出前后等位置，因此拦截器的使用具有更大的弹性，所以在Spring架构的程序中推荐优先使用拦截器
...
## 5. SpringBoot中用到的设计模式

>   *   **单例模式**：Spring中的大多数Bean都是单例Bean
>   *   **简单工厂**：Spring中的BeanFactory就是简单工厂模式的体现，根据传入一个唯一的标识来获得Bean对象
>   *   **工厂模式**：Spring创建Bean就是通过工厂模式创建的
>   *   **代理模式**：SpringAOP使用了动态代理
>   *   **模板模式**：用来解决重复代码的问题，例如redisTemplate、jdbcTemplate
>   *   **观察者模式**：当一个对象的状态发生改变时，所有依赖它的对象都会得到通知
>   *   **策略模式**：Bean的实例化包含原生对象的实例化和代理对象的实例化，不同的实例化内部逻辑不一样
>   *   **装饰器模式**：一些以Wrapper命名的类都是装饰器模式。
