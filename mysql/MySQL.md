...
>   1.   脏写：事务A修改了事务B修改过但尚未提交的数据，一般绝对不允许发送
...
>        *   MyISAM的优点是访问的速度快，但不支持事务、行级锁、外键，并且系统崩溃后无法安全恢复适用于对事务没要求且以Insert、Select为主的应用
...
>   为什么会产生主从同步延时问题：
>
>   1.   MySQL的主从复制都是单线程的操作，主线程写入Binlog日志时是顺序写入，效率很高。slave重做中继日志时是随机IO，成本高很多，所有SQL Thread线程的速度赶不上主库写Binlog的速度，就会产生主从延迟
