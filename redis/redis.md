...
>   *   Caw eine
>
>   Caw eine是一个基于 Java8 开发的提供了近乎最佳命中率的高性能的缓存库。
>
>   *   Memcached
>
...
>   **分布式锁**，是控制分布式系统不同进程共同访问共享资源的一种锁的实现。秒杀下单、抢红包等等业务场景，都需要用到分布式锁。一般使用的是redis的 setnx + expire 两个命令共同组合实现。redis实现分布式锁的原理：就是判断一个key是否在redis中存在；如果存在则说明加锁失败，如果不存在且 set命令执行成功则说明加锁成功。
